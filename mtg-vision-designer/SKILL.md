...
- `references/mechanics.md` — Keyword/ability word/named mechanic types, parasitism, complexity budget (the *theory*)
...
- `references/color-pie.md` — Vision-time color pie reference: identity statements, hard "no" lines, per-effect P/S/T tables, recent shifts (2017→2025), how to color-assign a new mechanic, UB color pie compensation
- `references/templating.md` — Quick reference for writing prototype rules text: 2024 "enters" / "this creature" templating, trigger words, replacement effects vs. triggers, targeting, common verb shifts, the "another" trap, vision-time priorities
- `references/rate-and-cost.md` — Vanilla creature curve, common removal pricing per color, card draw rates, combat trick / pump pricing, anthem and ramp rates, mechanic-as-fan targets, mana curve per Limited deck, format speed targets, above/below-curve markers
...
- `references/wisdom-catalog.md` — Failure stories (BFZ split identity, Ixalan insular archetypes, Born of the Gods invisible set, Ikoria overload, Kamigawa original vs. NEO, Strixhaven competing pillars, Avacyn Restored no common support), counterintuitive insights, evolved thinking, and named heuristics. Also includes 2024–2025 mechanics-era wisdom (Ward over Hexproof, Prowess demoted, Kindred renamed, predefined token palette, alt-cost saturation, Disguise/Cloak as morph upgrade). **Consult this when making vision decisions.**
- `references/mechanics-catalog.md` — Working inventory: evergreen keyword abilities and actions with reminder text, deciduous keywords, every named mechanic from 2024–2025 premier sets (MKM, OTJ, BLB, DSK, FDN, DFT, TDM, FIN), and static concepts (MDFCs, Sagas, Battles, Vehicles, predefined tokens, counters). **Consult this when selecting or naming mechanics, to avoid re-treading recent design space and to know what evergreens cost zero complexity budget.**
- `references/color-pie.md` — Vision-time color pie: per-color identity statements, hard "no" lines (no blue ramp, no red enchantment removal, etc.), P/S/T lookup tables for every major effect family, 2017→2025 shifts (Ward over Hexproof, Prowess demote, white-draw allowance, etc.), a color-assignment decision framework for new mechanics, and UB color-pie compensation guidance. **Consult before assigning colors to any new mechanic and before writing archetype payoff sketches.**
- `references/templating.md` — Quick reference for writing prototype rules text in `vision_cardfile.json`: the 2024 "enters" and "this creature" shifts, trigger words, replacement effects, targeting, activation cost ordering, common verb updates, and the most common vision-time templating bugs (the "another" trap, replacement-vs-trigger confusion). Defers to `mtg-editor` for the comprehensive manual. **Consult while writing card rules text.**
- `references/rate-and-cost.md` — Vanilla creature curve (P+T ≈ 2× mana value), common removal pricing per color (with Play Booster era efficiency increase noted), card draw / combat trick / pump / counterspell / token / land-ramp rates, mechanic-as-fan targets at common, drafted-deck mana curve, format speed targets. **Consult while costing cards and setting common counts for each mechanic.**
